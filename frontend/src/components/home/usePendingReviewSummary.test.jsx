...
import {
  pendingSummarySessionKey as serverSessionKey,
...
const pendingSummarySessionKey = (user) => serverSessionKey(user, "Validation");
...
function Probe({ context }) {
  const state = usePendingReviewSummary(context);
  return (
    <>
...
test("review verifies the explicit Validation role before and after its count without writing shared CSRF", async () => {
  readPendingReviewSummary.mockResolvedValue(summary());
  ...
  for (const args of readPendingSummarySession.mock.calls)
    expect(args[1]).toBe("Validation");
...
test("an older refresh cannot overwrite a newer successful response", async () => {
  ...
  expect(readPendingReviewSummary.mock.calls[0][0].aborted).toBe(true);
...
test.each([
  ...
  async (change) => {
    ...
    expect(read("count")).toBe("unknown");
    await waitFor(() =>
      expect(readPendingReviewSummary).toHaveBeenCalledTimes(3),
    );
...
test("role removal clears an in-flight count without requesting another summary", async () => {
  ...
  readPendingReviewSummary.mockReturnValue(pending.promise);
...
test("a CSRF change in another tab hides counts and rechecks the same identity without accepting an old response", async () => {
  ...
  expect(read("count")).toBe("unknown");
  expect(read("time")).toBe("never");
  expect(readPendingReviewSummary.mock.calls[1][0].aborted).toBe(true);
  await waitFor(() =>
    expect(readPendingReviewSummary).toHaveBeenCalledTimes(3),
  );
...
test("initial login accepts a different CSRF mask in storage and context without ordinary-render retries", async () => {
  localStorage.setItem("CSRF", "other-valid-mask");
  readPendingReviewSummary.mockResolvedValue(summary(25));
  const { rerender } = render(<Probe context={initialContext()} />);
...
test("a storage clear aborts an old post-verification and rejects a different account cookie", async () => {
  ...
  readPendingReviewSummary.mockResolvedValue(summary(99));
  render(<Probe context={initialContext()} />);
  await waitFor(() =>
    expect(readPendingSummarySession).toHaveBeenCalledTimes(2),
  );
...
test.each(["before", "after"])(
  ...
  async (phase) => {
    ...
    expect(read("count")).toBe("unknown");
    expect(read("time")).toBe("never");
    expect(readPendingReviewSummary).toHaveBeenCalledTimes(
      phase === "before" ? 0 : 1,
    );
  },
);
