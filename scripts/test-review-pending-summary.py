#!/usr/bin/env python3
"""Run A-02 review backend tests against a fresh synthetic PostgreSQL database."""
...
def main():
    ...
    output = Path(tempfile.mkdtemp(prefix="lis-a02-review-backend-"))
    database = "lis_a02_review_test_" + datetime.date.today().strftime("%Y%m%d") + "_" + secrets.token_hex(4)
    if not re.fullmatch(r"lis_a02_review_test_[0-9]{8}_[a-z0-9]+", database):
    ...
    try:
        ...
        stage = "copy_schema_only"
        tables = ("analysis", "sample_item", "sample", "test", "test_section", "result",
                  "test_result_component", "result_file", "result_file_seq", "test_result", "localization", "localization_value")
        ...
        stage = "configure_test_connection"
        ...
        env.update({"MAVEN_OPTS": env.get("MAVEN_OPTS", "") + " -Xms128m -Xmx1024m",
                    ...
                    "A02_REVIEW_TEST_DB_URL": "jdbc:postgresql://127.0.0.1:" + str(args.port) + "/" + database,
                    "A02_REVIEW_TEST_DB_USER": user, "A02_REVIEW_TEST_DB_PASSWORD": password})
        tests = ",".join(("ReviewSummaryFactoryTest", "ReviewScopeServiceTest", "ReviewPendingServiceTest",
                          "ReviewQueryContextServiceTest", "ReviewSubmissionServiceTest", "ReviewWriteGuardTest",
                          "AccessionValidationQueryControllerTest", "ReviewPendingRestContractTest",
                          "AccessionValidationProjectionTest", "PendingDashboardPagingTest", "UserServiceImplUnitTest",
                          "QCReleaseGateServiceTest", "ReviewPendingHibernateQueryTest", "ReviewPendingDatabaseIT",
                          "ReviewSubmissionPolicyDatabaseIT"))
        ...
        # Error messages can contain environment values; record only class/stage.
    ...
    return 1 if "failure_type" in record or "cleanup_failure_type" in record else record.get("maven_exit_code", 1)
