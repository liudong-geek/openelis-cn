...
| 历史版本/打印               | ISSUED/SUPERSEDED/VOIDED 可按原件完整性与权限读取；只有当前 ISSUED 可记录打印。打印接口记录的是取原件并计数的动作，**不是打印机已经出纸的硬件回执**，且不能凭别人增加的计数推断本次未知请求成功。        | S11:297–331、400–426；旧前端核查 R4 |
...
| 同管部分项目可录、新增项目无旧验收 | 准入按 analysis 判断，原正常项目与新增不允许项目分开；当前验收证据保留。                                                       | A-03 做清晰原因、可达处理入口和岗位提示，避免把整管无理由一刀切或把受阻项目当可处理总量。S14:74–94；旧前端核查 M6。                 |
...
本轮引用证据校验：54/54 与固定提交逐文件 blob 相同；其中 F2 使用上述固定提交摘取文件，其余 53 份使用核查时当前可读文件。仅为源码来源验证，不是功能测试。
