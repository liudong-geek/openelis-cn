...
public class ResultEntryWorklistServiceTest {
    ...
    public void reviewReturnsReenterQueueButReleasedOrPrintedRowsStayExcluded() {
        Analysis returned = new Analysis();
        returned.setId("43");
        Analysis released = new Analysis();
        released.setId("44");
        org.springframework.test.util.ReflectionTestUtils.setField(released, "releasedDate", new java.sql.Timestamp(1));
        Analysis printed = new Analysis();
        printed.setId("45");
        org.springframework.test.util.ReflectionTestUtils.setField(printed, "printedDate", new java.sql.Date(1));
        when(statusService.getStatusID(AnalysisStatus.NotStarted)).thenReturn("4");
        when(statusService.getStatusID(AnalysisStatus.BiologistRejected)).thenReturn("5");
