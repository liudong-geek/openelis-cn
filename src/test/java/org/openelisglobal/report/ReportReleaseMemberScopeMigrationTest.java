...
public class ReportReleaseMemberScopeMigrationTest {
    private static final Path MIGRATION = Path
            .of("src/main/resources/liquibase/3.5.x.x/086-report-release-member-scope.xml");
    ...
    public void migrationNeverBackfillsHistoricalMembershipAndRunsAfterDocumentScope() throws Exception {
        ...
        assertTrue(sql.contains("member_scope_json IS NULL AND member_scope_sha256 IS NULL"));
        assertTrue(sql.contains("report_document_id IS NOT NULL"));
        String base = Files.readString(Path.of("src/main/resources/liquibase/3.5.x.x/base.xml"));
        assertTrue(base.indexOf("085-report-document-release-scope.xml") < base
                .indexOf("086-report-release-member-scope.xml"));
    }
    ...
    public void rollbackCannotDropStoredMembershipEvidence() throws Exception {
        ...
        assertTrue(rollback.contains("member_scope_json IS NOT NULL OR member_scope_sha256 IS NOT NULL"));
