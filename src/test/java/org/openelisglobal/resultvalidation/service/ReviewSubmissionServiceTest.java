...
/** SIM transaction ordering and dispatch boundaries; does not simulate PostgreSQL rollback. */
public class ReviewSubmissionServiceTest {
    ...
    @Before public void setup() {
        fixture = new ReviewWriteGuardTest(); fixture.setup();
        when(fixture.states.analysis("101")).thenAnswer(call -> new org.openelisglobal.resultvalidation.dao.ReviewSaveStateDAO.AnalysisState(
                "101", "401", "201", "301", "SIM-RESULT-301", "501", fixture.analysis.getStatusId(),
                String.valueOf(fixture.analysis.getLastupdated().getTime()), fixture.analysis.getReleasedDate() != null,
                fixture.analysis.getPrintedDate() != null));
        analyses = mock(AnalysisService.class); signatures = mock(ElectronicSignatureService.class);
        notes = mock(NoteService.class); fhir = mock(FhirTransformService.class); notifications = mock(TestNotificationService.class);
        var users = mock(SystemUserService.class);
        var user = new SystemUser(); user.setId("801"); user.setLoginName("sim-reviewer"); user.setIsActive("Y");
        when(users.get("801")).thenReturn(user);
        service = spy(new ReviewSubmissionService(fixture.guard, analyses, fixture.statuses, notes,
                mock(SampleService.class), mock(SampleHumanService.class), signatures, users, fixture.specimens, fhir, notifications));
        updater = mock(IResultUpdate.class); doReturn(List.of(updater)).when(service).registeredUpdaters();
        request = new MockHttpServletRequest();
        var data = new UserSessionData(); data.setSytemUserId(801); data.setLoginName("sim-reviewer");
        request.getSession().setAttribute(IActionConstants.USER_SESSION_DATA, data);
        credentials = new ReviewSignature(); credentials.setUsername("sim-reviewer"); credentials.setPassword("SIM-only-secret");
        when(notes.createSavableNote(any(), any(), anyString(), anyString(), eq("801"))).thenAnswer(call -> {
            Note note = new Note(); note.setText(call.getArgument(2)); return note;
        });
        ...
        var signature = new ElectronicSignature(); signature.setId(901L);
        when(signatures.executeSignatureForSnapshot(anyString(), anyString(), any(), anyString(), anyLong(), any(), any(), any(), anyString())).thenReturn(signature);
    }
    @After public void cleanup() { fixture.cleanup(); }
    private void save() { service.save(request, "801", List.of(fixture.row), credentials); }

    @Test public void signatureIsBoundToStoredValueAndVersionBeforeAnalysisMutation() {
        save();
        ...
        order.verify(signatures).executeSignatureForSnapshot(eq("sim-reviewer"), eq("SIM-only-secret"),
                eq(SignatureMeaning.VALIDATED_AND_RELEASED), eq("ANALYSIS"), eq(101L), isNull(), any(), any(), content.capture());
    ...
    @Test public void staleEvidenceFailsBeforeSignatureCreation() {
    ...
    @Test public void returnHasReasonInSignatureAndInternalAuditAndNoReleasedDate() {
        fixture.row.setIsAccepted(false); fixture.row.setIsRejected(true); fixture.row.setNote("模拟退回复检");
        save();
        ...
        assertEquals("21", fixture.analysis.getStatusId()); assertNull(fixture.analysis.getReleasedDate());
        verify(notes).createSavableNote(eq(fixture.analysis), eq(org.openelisglobal.note.service.NoteServiceImpl.NoteType.INTERNAL),
                contains("signatureId=901"), eq("Review Decision"), eq("801"));
    }
    ...
    @Test public void notificationsAndExternalUpdatesOnlyRunAfterSuccessfulCommit() throws Exception {
        save();
        verifyZeroInteractions(fhir, notifications);
        verify(updater).transactionalUpdate(any()); verify(updater, never()).postTransactionalCommitUpdate(any());
    ...
    @Test public void updaterFailureIsPropagatedWithoutSchedulingExternalEffects() {
    ...
    @Test public void callbackCannotChangeReviewedResultValueOrRevokeAccess() {
        doAnswer(call -> { fixture.result.setValue("unexpected"); return null; }).when(updater).transactionalUpdate(any());
        assertEquals(409, assertThrows(ResponseStatusException.class, this::save).getStatusCode().value());
        verifyZeroInteractions(fhir, notifications);
    }
    ...
    @Test public void passwordAndSignatureRequestAreNeverSerializedBack() throws Exception {
        var mapper = new com.fasterxml.jackson.databind.ObjectMapper();
        var form = new ResultValidationForm(); form.setReviewSignature(credentials);
        ...
        var parsed = mapper.readValue("{\"reviewSignature\":{\"username\":\"sim-reviewer\",\"password\":\"SIM-only\"}}", ResultValidationForm.class);
