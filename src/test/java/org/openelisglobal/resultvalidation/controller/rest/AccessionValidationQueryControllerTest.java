...
public class AccessionValidationQueryControllerTest {
    ...
    @Test public void actualHttpMappingPreservesStaleAndForbiddenStatuses() throws Exception {
        ...
        for (HttpStatus error : new HttpStatus[] { HttpStatus.CONFLICT, HttpStatus.FORBIDDEN }) {
            doThrow(new ResponseStatusException(error)).when(contexts)
                    .page(any(), eq("7"), any(), eq("SIM-query-A"), eq("1"));
            mvc.perform(get("/rest/AccessionValidation").session((MockHttpSession) request.getSession())
                    .param("accessionNumber", "SIM-A").param("doRange", "false")
                    .param("queryId", "SIM-query-A").param("page", "1"))
                    .andExpect(status().is(error.value()));
        }
        verifyZeroInteractions(persistence);
    }

    @Test public void missingSampleInitialGetStillCreatesAnEmptyIndependentQuery() throws Exception {
        ...
        try {
            ...
            ResultValidationForm response = controller.showAccessionValidationRange(request, "SIM-missing", null, null, false);
            verify(samples).getSampleByAccessionNumber("SIM-missing");
            verify(contexts).create(eq(request.getSession()), eq("7"), same(response), eq(java.util.List.of()), eq(false));
    ...
    @Test public void getPageForwardsQueryIdentityAndOriginalCriteria() throws Exception {
    ...
    @Test public void missingPageTokenNeverFallsBackToLegacySessionCache() throws Exception {
        request.getSession().setAttribute(IActionConstants.RESULTS_SESSION_CACHE, java.util.List.of("SIM-old"));
        request.setParameter("page", "1");
        doThrow(new ResponseStatusException(HttpStatus.CONFLICT)).when(contexts)
                .page(any(), anyString(), any(), isNull(), eq("1"));
        try { controller.showAccessionValidationRange(request, "SIM-A", null, null, false); fail(); }
        catch (ResponseStatusException e) { assertEquals(409, e.getStatusCode().value()); }
        verifyZeroInteractions(persistence);
    }

    @Test public void invalidBodyStopsBeforeContextConsumptionOrPersistence() throws Exception {
        ResultValidationForm form = new ResultValidationForm();
        BeanPropertyBindingResult errors = new BeanPropertyBindingResult(form, "form");
        errors.reject("SIM-invalid");
        try { controller.showAccessionValidationRangeSave(request, form, errors); fail(); }
        catch (ResponseStatusException e) { assertEquals(400, e.getStatusCode().value()); }
        verifyZeroInteractions(contexts, persistence);
    }

    @Test public void unsupportedLegacyPostPagingIsRejectedBeforeAnyCacheAccess() throws Exception {
        request.setParameter("pageResults", "true");
        ResultValidationForm form = new ResultValidationForm();
        try { controller.showAccessionValidationRangeSave(request, form, new BeanPropertyBindingResult(form, "form")); fail(); }
        catch (ResponseStatusException e) { assertEquals(409, e.getStatusCode().value()); }
        verifyZeroInteractions(contexts, persistence);
    }

    @Test public void postStaleContextOrRevokedPermissionCannotReachPersistence() throws Exception {
        for (HttpStatus status : new HttpStatus[] { HttpStatus.CONFLICT, HttpStatus.FORBIDDEN }) {
            ...
            try { controller.showAccessionValidationRangeSave(request, form, new BeanPropertyBindingResult(form, "form")); fail(); }
            catch (ResponseStatusException e) { assertEquals(status.value(), e.getStatusCode().value()); }
        }
        verifyZeroInteractions(persistence);
    }
}
