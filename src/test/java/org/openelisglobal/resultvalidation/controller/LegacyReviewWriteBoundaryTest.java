...
public class LegacyReviewWriteBoundaryTest {
    @Test public void oldManualFormsRejectBeforeConsumingCacheOrMutatingEntities() throws Exception {
        ...
        var byTest = new ResultValidationController(analyses, mock(TestResultService.class), mock(SampleHumanService.class),
                mock(DocumentTrackService.class), mock(TestSectionService.class), mock(SystemUserService.class),
                references, documents, persist, mock(NoteService.class), mock(FhirTransformService.class));
        var byAccession = new AccessionValidationRangeController(analyses, mock(TestResultService.class), mock(SampleHumanService.class),
                mock(DocumentTrackService.class), mock(TestSectionService.class), mock(SystemUserService.class),
                references, documents, persist, mock(NoteService.class), mock(FhirTransformService.class));
        ...
        assertEquals(409, assertThrows(ResponseStatusException.class,
                () -> byAccession.showAccessionValidationRangeSave(request, form, errors, null)).getStatusCode().value());
